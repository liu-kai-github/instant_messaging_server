...
module.exports = async ([username, password]) => {
    ...
    if (userOnlineInfo.length !== 0) {
        // 如果 token 还没有过期
        if ((Date.now() - userOnlineInfo[0].recent_access_time) <= tokenTimeLimit) {
            // 更新一下最新的登录时间
            await executeSQL('UPDATE user_online SET recent_access_time = ? WHERE user_id = ?;', [Date.now(), username]);
            return [
                null,
                {
                    username,
        ...
        await clearUserOnlineInfo(username);
    }
